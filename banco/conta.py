...
class Conta:
    ...
    __slots__ = ['_numero', '_titular', '_saldo',
                 '_limite', '_historico', '_l_max', '_senha']
    ...
    def sacar(self, valor):
        if self._saldo >= 0:
            ...
            if (self._saldo < valor):
                ...
                return True
        self._historico.transacoes.append(
            'Tentativa de saque de R${} - SALDO INSUFICIENTE!'.format(valor))
        return False

    def extrato(self):
        uma_string = ''
        uma_string += ('\nConta: {} - Saldo: R${}'.format(self._numero,
                       self._saldo))+'\n'
        uma_string += self._historico.imprimir() + '\n'
        self._historico.transacoes.append(
            'Emissão de extrato-saldo de R${}\n'.format(self._saldo))
        return uma_string
    ...
    def pagarLimite(self, valor):
        ...
        if valor >= divida:
            ...
            self._historico.transacoes.append(
                'Pagamento de R${} do limite'.format(divida))

        else:
            self._limite += valor
            self._historico.transacoes.append(
                'Pagamento de R${} do limite'.format(valor))
